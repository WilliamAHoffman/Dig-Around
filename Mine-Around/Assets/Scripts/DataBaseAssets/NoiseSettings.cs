...
public class NoiseSettings : DatabaseAsset
{
    ...
    {
        int regularSeed = worldSeed.seed ^ GameRandomness.StableHash(name + "_regular");
        int warpSeed = worldSeed.seed ^ GameRandomness.StableHash(name + "_warp");

        noise = new FastNoiseLite(regularSeed);
        warpNoise = new FastNoiseLite(warpSeed);

        offsetX = (worldSeed.seed ^ GameRandomness.StableHash(name + "_offset_x")) % 10000;
        offsetY = (worldSeed.seed ^ GameRandomness.StableHash(name + "_offset_y")) % 10000;
    ...
    public float Sample(int worldX, int worldY)
    {
        ...
        if (noise == null)
        {
            Debug.LogError($"NoiseSettings '{name}' was sampled before CreateNoise() was called.", this);
            return 0f;
        }
