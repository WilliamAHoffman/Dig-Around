...
public class NoiseSettings : DatabaseAsset
{
    ...
    {
        int regularSeed = WorldSeed ^ GameRandomness.StableHash(name + "_regular");
        int warpSeed = WorldSeed ^ GameRandomness.StableHash(name + "_warp");

        noise = new FastNoiseLite(regularSeed);
        warpNoise = new FastNoiseLite(warpSeed);

        offsetX = (WorldSeed ^ GameRandomness.StableHash(name + "_offset_x")) % 10000;
        offsetY = (WorldSeed ^ GameRandomness.StableHash(name + "_offset_y")) % 10000;
